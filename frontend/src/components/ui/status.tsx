...
export type StatusProps = ComponentProps<typeof Badge> & {
  status: "online" | "offline" | "maintenance" | "degraded" | "open-to-work"
}
...
export const Status = ({
  ...
  <Badge
    className={cn(
      ...
      // Borderless open-to-work
      status === "open-to-work" &&
      "border-0 bg-transparent shadow-none px-0 py-0",
  ...
  <span className={cn("relative flex h-2 w-2", className)} {...(props as any)}>
    <span
      className={cn(
        ...
        "group-[.open-to-work]:bg-emerald-500",
      )}
    />
    <span
      className={cn(
  ...
  <span className={cn("text-muted-foreground", className)} {...(props as any)}>
    {children ?? (
      <>
        ...
        <span className="hidden group-[.open-to-work]:block">{config.status}</span>
      </>
    )}
  </span>
)
